const toFontName = (fontFamily, fontStyle, styles) => {
  if (fontFamily in styles) {
    const fontNames = styles[fontFamily].filter(fontName =>
      fontName.includes(`-${fontStyle.replace(/\s/g, '')}`),
    );
    if (fontNames.length > 0) {
      return fontNames.reduce((a, b) => (a.length <= b.length ? a : b));
    }
    if (fontStyle === 'Regular') {
      return styles[fontFamily].reduce((a, b) =>
...
export { toFontStyles, toFontName, enrichFontsData };
